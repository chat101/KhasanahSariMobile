...
export default function WorkOrderPengurangan() {
  // ====== DATE PICKER STATE ======
  ...
  const [batches, setBatches] = useState([]); // [{pengurangan_ke, detail:[...], sum_tong, sum_pcs}]
  const [items, setItems] = useState([]);     // fallback rekap (flat)
  ...
  const fetchData = useCallback(async () => {
    ...
    // Versi per-batch (loadPengurangan)
    const perBatchUrls = [
      `${base}/api/pengurangan/load?tanggal=${encodeURIComponent(tanggal)}`,
      `${base}/pengurangan/load?tanggal=${encodeURIComponent(tanggal)}`,
    ];
    // Fallback rekap per-produk (pengurangan)
    const rekapUrls = [
      `${base}/api/pengurangan/rekap?tanggal=${encodeURIComponent(tanggal)}&per_page=500&page=1`,
      `${base}/pengurangan/rekap?tanggal=${encodeURIComponent(tanggal)}&per_page=500&page=1`,
    ];
    ...
    try {
      // 1) Coba endpoint per-batch (loadPengurangan)
      let json = null;
      for (let i = 0; i < perBatchUrls.length; i++) {
        try {
          json = await tryFetchJson(perBatchUrls[i]);
          if (json) break;
        } catch {} // lanjut coba url berikutnya
      }

      if (json) {
        // Controller mengembalikan: { summary: { pengurangan_tables: [ { pengurangan_ke, detail[], sum_tong, sum_pcs } ] } }
        const rawGroups = json?.summary?.pengurangan_tables ?? [];
        const normalized = (Array.isArray(rawGroups) ? rawGroups : [])
          .map((g) => {
            ...
            return {
              pengurangan_ke: g?.pengurangan_ke ?? 0,
              detail,
              sum_tong: sumTong,
              sum_pcs: sumPcs,
            };
          })
          .sort((a, b) => (a.pengurangan_ke ?? 0) - (b.pengurangan_ke ?? 0));
        ...
        const flat = normalized
          ...
          .map((d, idx) => ({
            ...
            total_pengurangan: Number(d.qty_tong ?? 0),
            konversi_pengurangan: Number(d.konversi ?? 0),
          }));
        ...
        // 2) Fallback rekap per-produk
        let legacy = null;
        for (let i = 0; i < rekapUrls.length; i++) {
          try {
            legacy = await tryFetchJson(rekapUrls[i]);
            if (legacy) break;
          } catch {}
        }

        const mapped = (legacy?.data ?? []).map((it, idx) => ({
          id: String(it.mproducts_id ?? it.id ?? idx),
          nama: it.nama ?? "-",
          patokan: Number(it.patokan ?? 0),
          total_pengurangan: Number(it.total_pengurangan ?? it.qty_tong ?? 0),
          konversi_pengurangan: Number(it.konversi_pengurangan ?? it.konversi ?? 0),
        }));

        const metaSafe = {
          rows: legacy?.meta?.rows ?? mapped.length,
          sum_tong: legacy?.meta?.sum_tong ?? mapped.reduce((a, b) => a + (Number(b.total_pengurangan) || 0), 0),
          sum_pcs: legacy?.meta?.sum_pcs ?? mapped.reduce((a, b) => a + (Number(b.konversi_pengurangan) || 0), 0),
        };
  ...
  const DateHeader = useMemo(
    () => (
      <>
        <View style={s.topBar}>
          <Text style={s.title}>Pengurangan Produksi</Text>
        ...
        <View style={s.summaryWrap}>
          <View style={s.card}>
            ...
            <View style={s.cardBody}>
              <Text style={s.cardLabel}>Total Tong (Pengurangan)</Text>
  ...
  // ====== RENDER TABEL PER BATCH (baru) ======
  const renderBatchTable = (batch) => {
    const { pengurangan_ke, detail = [], sum_tong = 0, sum_pcs = 0 } = batch || {};
    return (
      <View key={`batch-${pengurangan_ke}`} style={s.batchCard}>
        <View style={s.batchHeader}>
          <Text style={s.batchTitle}>Pengurangan {pengurangan_ke}</Text>
          <View style={s.badge}>
        ...
        {detail.length === 0 ? (
          <View style={s.emptyWrap}><Text style={s.empty}>Tidak ada data pada pengurangan ini.</Text></View>
        ) : (
          detail.map((item, index) => (
            <View
              key={`${pengurangan_ke}-${item.mproducts_id ?? index}`}
  ...
  return (
    <View style={s.container}>
      ...
      {loading && !refreshing && !usingBatches && items.length === 0 ? (
        ...
        // Fallback rekap: satu tabel flat
        <FlatList
          data={items}
          ...
          renderItem={({ item, index }) => (
            <View style={[s.row, index % 2 === 0 ? s.rowEven : s.rowOdd]}>
              ...
              <Text style={[s.cell, s.colProduk]} numberOfLines={2}>{item.nama}</Text>
              <Text style={[s.cell, s.colRight, s.semibold, s.primaryTxt]}>
                {fmt(item.total_pengurangan, 1)}
              </Text>
              <Text style={[s.cell, s.colRight]}>{fmt(item.patokan, 0)}</Text>
              <Text style={[s.cell, s.colRight]}>
                {item.konversi_pengurangan === 0 ? "-" : fmt(item.konversi_pengurangan, 0)}
              </Text>
            </View>
          )}
          ListHeaderComponent={
