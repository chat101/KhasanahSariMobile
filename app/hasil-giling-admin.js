...
/* ===== Ambil peta realisasi untuk suatu divisi (key: mproducts_id) ===== */
const fetchDivisiMap = useCallback(
  async (date, targetDivisiId) => {
    try {
      const token = tokenRef.current || (await AsyncStorage.getItem(KEY_TOKEN));
      if (!token || !targetDivisiId) return {};

      const q = new URLSearchParams({
        date: String(date),
        only_net_positive: "true",
        include_totals: "false",
        divisi_id: String(targetDivisiId),
      }).toString();

      const r = await fetch(`${base}/api/hasil-giling?${q}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
      ...
      // Map mproducts_id -> realisasi angka
      const map = {};
      for (const it of arr) {
        map[String(it.mproducts_id)] =
          Number(it?.realisasi_divisi ?? it?.realisasi ?? 0) || 0;
      }
      return map;
    } catch {
      return {};
  ...
  const loadRejectSummary = useCallback(
    async (perintahIdArg) => {
      try {
        ...
        // ← tampilkan total REJECT dari SEMUA divisi (tanpa filter divisi_id)
        const q = new URLSearchParams({
  ...
  const fetchData = useCallback(
    async (d) => {
      ...
      try {
        ...
        // Ambil realisasi dari divisi lain untuk kolom Poprok (6) & Dekor (4)
        const [mapPoprok, mapDekor] = await Promise.all([
  ...
  const totalRejectQty = useMemo(() => {
    ...
    // total kolom baru: Poprok & Dekor
    const totalsExtra = useMemo(() => {
      const poprok = rows.reduce((a, b) => a + (Number(b?.poprok) || 0), 0);
...
const totalsPengalihan = useMemo(() => {
  ...
  // Header tabel (tanpa kolom Selisih)
  const TableHeader = (
    <View style={[s.row, s.headerRow]}>
      ...
      {/* Tong */}
      <Text style={[s.cell, s.colReal, s.headerText, s.center]}>Tong</Text>
      {/* Target setelah Tong */}
      <Text style={[s.cell, s.colTarget, s.headerText, s.center]}>Target</Text>
      {/* Real Giling sesudah Target */}
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      <View style={[s.row, index % 2 === 0 ? s.rowEven : s.rowOdd]}>
        ...
        {/* Tong (qty_total per produk) */}
        <Text style={[s.cell, s.colReal, s.center]}>
          {fmt(item?.qty_total || 0, 0)}
        </Text>

        {/* Target per produk */}
        <Text style={[s.cell, s.colTarget, s.center]}>
          {fmt(item?.target_giling || 0, 0)}
        </Text>

        {/* Real Giling (realisasi divisi) */}
        <Text style={[s.cell, s.colRight, s.semibold, s.primaryDarkTxt]}>
          {fmt(item?.realisasi_divisi ?? item?.realisasi ?? 0, 0)}
        </Text>

        {/* Kolom baru: Poprok & Dekor (read-only dari data) */}
        <Text style={[s.cell, s.colPop, s.center]}>
          {fmt(item?.poprok || 0, 0)}
        </Text>
...
<Text style={[s.cell, s.colPengalihan, s.center]}>
  {item?.pengalihan_keluar > 0 ? (
    ...
    <Text style={{ color: COLORS.sub }}>—</Text>
        ...
        <Text style={[s.cell, s.colReject, s.center]}>
          {fmt(rejectSummary[String(mId)]?.qty || 0, 0)}
        </Text>
      </View>
    );
  };

  // Footer (tampil total tanpa kolom Selisih)
  const TableFooter = (
    <View style={[s.row, s.footerRow]}>
      ...
      {/* Total Tong */}
      <Text style={[s.cell, s.colReal, s.center]}>
        {fmt(totals.qty_total, 0)}
      </Text>

      {/* Total Target */}
      <Text style={[s.cell, s.colTarget, s.center]}>
        {fmt(totals.target_giling, 0)}
      </Text>

      {/* Total Real Giling */}
      <Text style={[s.cell, s.colRight, s.semibold, s.primaryDarkTxt]}>
        {fmt(totals.realisasi, 0)}
      </Text>

      <Text style={[s.cell, s.colPop, s.center, s.semibold]}>
        {fmt(totalsExtra.poprok, 0)}
      </Text>
    ...
    >
      <View style={[s.modalOverlayTop, { paddingTop: insets.top + MODAL_TOP }]}>
        <View style={s.modalCardTop}>
          ...
          >
            {/* (DINONAKTIFKAN – tidak ditampilkan lagi di UI utama) */}
          </KeyboardAvoidingView>

          {/* ==== HISTORY LIST (DINONAKTIFKAN DI UI) ==== */}
  ...
  return (
    ...
    >
      ...
      {/* Modals tidak dipasang lagi di UI */}
      {/* {RejectModal}
      {ListRejectModal} */}
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  hScrollContent: { minWidth: 500 }, // tambah lebar minimum agar muat kolom baru
